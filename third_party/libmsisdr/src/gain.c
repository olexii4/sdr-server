...
int msisdr_set_gain(msisdr_dev_t *p)
{
...
#endif
    /* Re-enable gain control every time gain is set. RSP1A firmware resets
     * register 0x08 when streaming starts, disabling gain control. Writing
     * 0xf380 before the gain registers restores it. Fix by SM5BSZ 2014-12-05,
     * re-enabled to support RSP1A hardware. */
    ...
    if (p->band == MSISDR_BAND_AM1)
    {
        reg1 |= (p->gain_reduction_mixbuffer & 0x03) << 10;
    }
    else if (p->band == MSISDR_BAND_AM2)
    {
        #if MSISDR_DEBUG >= 1
            fprintf(stderr, "msisdr_set_gain: gain_reduction_mixbuffer: %d\n", p->gain_reduction_mixbuffer);
        #endif
    ...
    if ((p->band == MSISDR_BAND_AM1) || (p->band == MSISDR_BAND_AM2))
    {
        reg1 |= 0x0 << 13;
    }
    else
    ...
    reg1 |= MSISDR_DC_OFFSET_CALIBRATION_PERIODIC2 << 14;
    reg1 |= MSISDR_DC_OFFSET_CALIBRATION_SPEEDUP_OFF << 17;
    msisdr_write_reg(p, 0x09, reg1);

    /* DC Offset Calibration setup */
    reg6 |= 0x1F << 4;
    reg6 |= 0x800 << 10;
    msisdr_write_reg(p, 0x09, reg6);
//// set to 0xf300 to select AM input added Dec 5 2014 SM5BSZ
//    if (p->freq < 50000000)
//      {
//      msisdr_write_reg(p, 0x08, 0xf300);
//      }
//    else
//      {
//      if (p->freq >= 108000000)
...
////        msisdr_write_reg(p, 0x08, 0xf3ff);
//        }
//      }
...
int msisdr_set_tuner_gain(msisdr_dev_t *p, int gain)
{
    p->gain = gain;
    /*
    ...
    return msisdr_set_gain(p);

    gain_auto: return msisdr_set_tuner_gain_mode(p, 0);
}

int msisdr_get_tuner_gain(msisdr_dev_t *p)
{
    int gain = 0;
...
int msisdr_set_tuner_gain_mode(msisdr_dev_t *p, int mode)
{
//    if (!mode) {
//        p->gain = -1;
//#if MSISDR_DEBUG >= 1
...
//#endif
//        msisdr_write_reg(p, 0x09, 0x014281);
//        msisdr_write_reg(p, 0x09, 0x3FFFF6);
//    } else if (p->gain < 0) {
//#if MSISDR_DEBUG >= 1
...
int msisdr_get_tuner_gain_mode(msisdr_dev_t *p)
{
    return (p->gain < 0) ? 0 : 1;
}
 ...
 * 0x11   18 dB   24 dB
 */
int msisdr_set_mixer_gain(msisdr_dev_t *p, int gain)
{
...
int msisdr_set_mixbuffer_gain(msisdr_dev_t *p, int gain)
{
    if (gain < 0) {
        fprintf(stderr, "ERROR: msisdr_set_mixbuffer_gain: negative number provided: %d\n", gain);
        return -1;
    }
...
int msisdr_set_lna_gain(msisdr_dev_t *p, int gain)
{
...
int msisdr_set_baseband_gain(msisdr_dev_t *p, int gain)
{
...
int msisdr_get_mixer_gain(msisdr_dev_t *p)
{
    return p->gain_reduction_mixer ? 0 : 19;
}

int msisdr_get_mixbuffer_gain(msisdr_dev_t *p)
{
...
int msisdr_get_lna_gain(msisdr_dev_t *p)
{
    return p->gain_reduction_lna ? 0 : 24;
}

int msisdr_get_baseband_gain(msisdr_dev_t *p)
{
