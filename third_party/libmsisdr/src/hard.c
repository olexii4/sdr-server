...
int msisdr_set_hard(msisdr_dev_t *p)
{
	int streaming = 0;
	...
	if (p->async_status == MSISDR_ASYNC_RUNNING)
	{
		streaming = 1;

		if ((msisdr_stop_async(p) < 0) || (msisdr_adc_stop(p) < 0)) {
			goto failed;
		}
	}
	...
	/* omezení rozsahu */
	/* limit the scope of */
	if (p->rate > MSISDR_SAMPLE_RATE_MAX)
	{
		fprintf(stderr, "can't set rate %u, setting maximum rate: %d\n", p->rate, MSISDR_SAMPLE_RATE_MAX);
		p->rate = MSISDR_SAMPLE_RATE_MAX;
	}
	else if (p->rate < MSISDR_SAMPLE_RATE_MIN)
	{
		fprintf(stderr, "can't set rate %u, setting minimum rate: %d\n", p->rate, MSISDR_SAMPLE_RATE_MIN);
		p->rate = MSISDR_SAMPLE_RATE_MIN;
	}

	/* automatická volba formátu */
	/* automatic choice format */
	if (p->format_auto == MSISDR_FORMAT_AUTO_ON)
	{
		if (p->rate <= 6048000) {
			p->format = MSISDR_FORMAT_252_S16;
		} else if (p->rate <= 8064000) {
			p->format = MSISDR_FORMAT_336_S16;
		} else if (p->rate <= 9216000) {
			p->format = MSISDR_FORMAT_384_S16;
		} else {
			p->format = MSISDR_FORMAT_504_S16;
		}
	}
...
#endif
		msisdr_write_reg(p, 0x07, 0x000094);
		p->addr = 252 + 2;
		break;
...
#endif
		msisdr_write_reg(p, 0x07, 0x000085);
		p->addr = 336 + 2;
		break;
...
#endif
		msisdr_write_reg(p, 0x07, 0x0000a5);
		p->addr = 384 + 2;
		break;
...
#endif
		msisdr_write_reg(p, 0x07, 0x000c94);
		p->addr = 504 + 2;
		break;
	}
	...
	msisdr_write_reg(p, 0x04, reg4);
	msisdr_write_reg(p, 0x03, reg3);

	/* opětovné spuštění streamu */
	/* restart stream */
	if ((streaming) && (msisdr_start_async(p) < 0)) {
		goto failed;
	}
...
int msisdr_set_sample_rate(msisdr_dev_t *p, uint32_t rate)
{
	p->rate = rate;

	return msisdr_set_hard(p);
}

uint32_t msisdr_get_sample_rate(msisdr_dev_t *p)
{
	return p->rate;
}

int msisdr_set_sample_format(msisdr_dev_t *p, const char *v)
{
	if (!strcmp(v, "AUTO"))
	{
		p->format_auto = MSISDR_FORMAT_AUTO_ON;
	}
	else
	{
		p->format_auto = MSISDR_FORMAT_AUTO_OFF;
		if (!strcmp(v, "252_S16")) {
			p->format = MSISDR_FORMAT_252_S16;
...
const char *msisdr_get_sample_format(msisdr_dev_t *p)
{
	if (p->format_auto == MSISDR_FORMAT_AUTO_ON) {
		return "AUTO";
	}
